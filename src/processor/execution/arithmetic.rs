...
impl CPU {
    fn execute_adc_imm(&mut self, memory: &mut MEM) {
        ...
        let carry = (self.A.value as u16 + memory_value as u16) > 0xFF;
        let prev_a = self.A;
        self.A += Wrapped::byte(memory_value as isize);
        if self.C {self.A += 1};
        self.C = carry;
        self.V = ((self.A.value as u8) & 0b_1000_0000) != ((prev_a.value as u8) & 0b_1000_0000);
        self.Z = self.A == 0;
...
impl CPU {
    fn execute_sbc_imm(&mut self, memory: &mut MEM) {
        let memory_value = memory.read(self.next_pc(), 1) as u8;
        let mut temp = (self.A.value as i8) as i32;
        self.A -= Wrapped::byte(memory_value as isize);
        if !self.C {self.A -= 1};
...
impl CPU {
    fn execute_cmp_imm(&mut self, memory: &mut MEM) {
        ...
        self.C = self.A.value >= value.0 as isize;
        self.Z = self.A == value.0 as isize;
        self.N = (((self.A - value.0 as isize).value as u8) & 0b_1000_0000) != 0;
        self.PC += 2;
    }
...
impl CPU {
    fn execute_cpx_imm(&mut self, memory: &mut MEM) {
        let value = Wrapping(memory.read(self.next_pc(), 1) as u8);
        self.C = self.X.value >= value.0 as isize;
        self.Z = self.X == value.0 as isize;
        self.N = (((self.X - value.0 as isize).value as u8) & 0b_1000_0000) != 0;
        self.PC += 2;
    }
...
impl CPU {
    fn execute_cpy_imm(&mut self, memory: &mut MEM) {
        let value = Wrapping(memory.read(self.next_pc(), 1) as u8);
        self.C = self.Y.value >= value.0 as isize;
        self.Z = self.Y == value.0 as isize;
        self.N = (((self.Y - value.0 as isize).value as u8) & 0b_1000_0000) != 0;
        self.PC += 2;
    }
