...
impl CPU {
    fn execute_asl_acc(&mut self) {
        let value = self.A.value as u8;
        self.C = value & 0b_1000_0000 != 0;
        self.N = value & 0b_0100_0000 != 0;
        self.A = Wrapped::byte(((value << 1) & 0b_1111_1110) as isize);
        self.Z = self.A == 0;
        self.PC += 1;
    }
...
impl CPU {
    fn execute_lsr_acc(&mut self) {
        ...
        self.N = false;
        self.A = Wrapped::byte(((value >> 1) & 0b_0111_1111) as isize);
        self.PC += 1;
    }
...
impl CPU {
    fn execute_rol_acc(&mut self) {
        ...
        self.A = Wrapped::byte(temp as isize);
        self.PC += 1;
    }
