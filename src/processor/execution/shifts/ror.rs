...
impl CPU {
    pub fn execute_ror(&mut self, mode: MemoryMode, memory: &mut MEM) {
        match mode {
            MemoryMode::Acc       => self.execute_ror_acc(memory),
...
macro_rules! ror {
    ($cpu:ident, $instruction:ident, $memory:ident) => {{
        ...
        let new_carry = (value & 0b_0000_0001) != 0;
        let result = (value >> 1) + ($cpu.C as u8) * 0b_1000_0000;
        $cpu.C = new_carry;
        $cpu.Z = result == 0;
...
// ROR IMPL
impl CPU {
    fn execute_ror_acc(&mut self, memory: &mut MEM) {
        let inst = Instruction::get_acc(&self, memory);
        inst.log(&self, "ROR");
        ror!(self, inst, memory);
        self.increment_pc(1);
    }
...
mod ror_tests {
    ...
    fn get_result_from_value(value: u8, carry: bool) -> ExpectedResult {
        ...
        ExpectedResult {
            result,
            result_carry: (value & 0b_0000_0001) != 0,
    ...
    proptest! {
        #[test]
        fn test_ror_acc(value in 0x00u8..=0xFF, carry: bool) {
            ...
            test_cpu.execute_ror_acc(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_ror_zeropage(low_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF, carry: bool) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            test_cpu.C = carry;
            ...
            test_cpu.execute_ror_zpg(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_ror_zeropage_x(low_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF, x_value in 0x00u8..=0xFF, carry: bool) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            ...
            test_cpu.execute_ror_zpgx(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_ror_absolute(low_byte in 0x00u8..=0xFF, high_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF, carry: bool) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            test_cpu.C = carry;
            if high_byte == 0x00 {
            ...
            test_cpu.execute_ror_abs(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_ror_absolute_x(low_byte in 0x00u8..=0xFF, high_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF, x_value in 0x00u8..=0xFF, carry: bool) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            test_cpu.store_x(x_value);
            test_cpu.C = carry;
            ...
            test_cpu.execute_ror_absx(&mut memory);
