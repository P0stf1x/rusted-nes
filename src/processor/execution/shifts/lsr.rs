...
impl CPU {
    pub fn execute_lsr(&mut self, mode: MemoryMode, memory: &mut MEM) {
        match mode {
            MemoryMode::Acc       => self.execute_lsr_acc(memory),
...
// LSR IMPL
impl CPU {
    fn execute_lsr_acc(&mut self, memory: &mut MEM) {
        let inst = Instruction::get_acc(&self, memory);
        inst.log(&self, "LSR");
        lsr!(self, inst, memory);
        self.increment_pc(1);
    }
...
mod lsr_tests {
    ...
    proptest! {
        #[test]
        fn test_lsr_acc(value in 0x00u8..=0xFF) {
            ...
            test_cpu.execute_lsr_acc(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_lsr_zeropage(low_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            ...
            test_cpu.execute_lsr_zpg(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_lsr_zeropage_x(low_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF, x_value in 0x00u8..=0xFF) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            ...
            test_cpu.execute_lsr_zpgx(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_lsr_absolute(low_byte in 0x00u8..=0xFF, high_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            if high_byte == 0x00 {
            ...
            test_cpu.execute_lsr_abs(&mut memory);
    ...
    proptest! {
        #[test]
        fn test_lsr_absolute_x(low_byte in 0x00u8..=0xFF, high_byte in 0x00u8..=0xFF, value in 0x00u8..=0xFF, x_value in 0x00u8..=0xFF) {
            let mut test_cpu: CPU = CPU::new();
            let mut memory: MEM = MEM::new(MEMORY_SIZE);
            test_cpu.store_x(x_value);
            if high_byte == 0x00 {
            ...
            test_cpu.execute_lsr_absx(&mut memory);
