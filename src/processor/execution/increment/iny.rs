...
impl CPU {
    pub fn execute_iny(&mut self, mode: MemoryMode, memory: &mut MEM) {
        match mode {
            MemoryMode::Implicit  => self.execute_iny_imp(memory),
...
impl CPU {
    fn execute_iny_imp(&mut self, memory: &mut MEM) {
...
mod iny_tests {
    ...
    fn test_iny() {
        ...
        test_cpu.execute_iny_imp(&mut test_memory);
